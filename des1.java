...
public class des1 {
  ...
  /*
   * This is the inverted s-box initialization as a matrix
   */
  static char invs[][] = 
    {
  ...
  */
     ...
     * 
     * Input
     *    pTextHex - The ciphertext we want to decrypt (in Hex form)
     *    keyHex - The key we are using to decrypt the ciphertext
     *    
     * Output
     *    cTextHex - The plaintext we originally encrypted
     *    
     * This method takes a ciphertext block of 4x4 and combines it with the
    ...
    public String aesDecrypt(String pTextHex, String[][] keyHex){
      ...
      cTextHex = pTextHexMat;
      
      
      // Start the decryption (encryption backwards)
      for(int i = (rounds); i >= 1; i--){
        ...
        if (i != 0){
          ...
          if (i != rounds) cTextHex = aesMixColumn(cTextHex);
          cTextHex = aesShiftRow(cTextHex);
          cTextHex = aesInvNibbleSub(cTextHex);
        }
      }
      roundKey = K;
      cTextHex = aesStateXOR(cTextHex, roundKey);
  ...
  public String[][] aesStateXOR(String[][] sHex, String[][] keyHex){
     ...
     * Inverted Sbox substitution, outputting the result.
     */
    
    public String[][] aesInvNibbleSub(String[][] inStateHex){
      String[][] outStateHex = new String[4][4];
      for(int i = 0; i < 4; i++){
        for (int j = 0; j < 4; j++){
          outStateHex[j][i] = aesInvSbox(inStateHex[j][i]);
        }
      }
      return outStateHex;
    }
  ...
  for (int k = 0; k < 4; k++){
     ...
     * 
     * Input
     *    inHex - The single byte hex value to be used in the inverted S-box lookup
     * 
     * Output
     *    result - The corresponding single byte hex value gotten from the inverted S-box
     *    
     *    
     * This method takes a hex pair string argument and performs the inverted S-box 
     * lookup by splitting the pair into 2 separate characters, converting them to their
     * decimal value, and matching the corresponding pair on the table.
     */

      private String aesInvSbox(String inHex){
        int slookup1 = hex2decimal(inHex.substring(0, 1));
        int slookup2 = hex2decimal(inHex.substring(1, inHex.length()));
        String result = Integer.toHexString(invs[slookup1][slookup2]);
        if(result.length() < 2){    // Clunky way to temporarily fix the 0 trimming problem
          result = "0" + result;
      }
      return result;
